class Program
{
    static void Main()
    {
        ...
        var summaries = repo.GetTopProductByRegionAndMonth();

        Console.WriteLine("=== TOP PRODUCT BY REGION AND MONTH ===\n");

        foreach (var s in summaries)
        {
            Console.WriteLine($"Region: {s.RegionName}");
            Console.WriteLine($"  Month: {s.Month}");
            Console.WriteLine($"  Top Product: {s.ProductName}");
            Console.WriteLine($"  Total Quantity Sold: {s.TotalQuantitySold}\n");
        }
...
public class SalesRepository
{
    ...
    public List<RegionMonthlyTopProductSummary> GetTopProductByRegionAndMonth()
    {
        ...
        /*
         === TOP PRODUCT BY REGION AND MONTH ===
        ...
        var result = SalesRecords
                    ...
                    .Select(g =>
                    {
                        var regionName = Regions.First(r => r.Id == g.Key.RegionId).Name;
                        var month = new DateTime(g.Key.Month.Year, g.Key.Month.Month, 1).ToString("MMMM yyyy");
                        var TopProduct = g.GroupBy(s => s.ProductId)
                                            .Select(records => new
                                            {
                                                NameProduct = Products.First(p => p.Id == records.Key).Name,
                                                TotalQuantity = records.Sum(record => record.QuantitySold)
                                            })
                        ...
                        return new RegionMonthlyTopProductSummary
                        {
                            ...
                            TotalQuantitySold = TopProduct.TotalQuantity,
                        };
                    })
    ...
    public List<RegionMonthlyTopProductSummary> GetTopProductByRegionAndMonth_2()
    {
